...
class IndexedCachedDataset(IndexedDataset):
    ...
    def __getitem__(self, i):
        ...
        # tmp = self.cache[ptx: ptx + a.size]
        np.copyto(a, np.reshape(np.array(self.cache[ptx: ptx + a.size]), np.shape(a)))
        item = torch.from_numpy(a).long()
        if self.fix_lua_indexing:
            item -= 1  # subtract 1 for 0-based indexing
        return item
...
class IndexedRawTextDataset(torch.utils.data.Dataset):
    ...
    def read_data(self, path, dictionary):
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if 'edge' not in path and '.mask' not in path and '.position' not in path:
    ...
    def get_original_text_for_hie(self, i):
        self.check_index(i)
        res = self.lines[i]
        res = [word for word in res.split() if word != 'sentence_separator_special_tag'
               and word != 'story_separator_special_tag']
        res = ' '.join(res)
        return res
        # return self.lines[i]
