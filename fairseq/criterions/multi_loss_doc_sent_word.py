...
@register_criterion('multi_loss_doc_sent_word')
class MultiLossCriterion(FairseqCriterion):

    def __init__(self, args, task):
        super().__init__(args, task)
        self.MultiMarginLoss = torch.nn.MultiMarginLoss(margin=0.0001)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        net_output, sentence_decoder_out, doc_decoder_out = model(**sample['net_input'])
        loss, _ = self.compute_loss(model, net_output, sample, reduce=reduce)
        sentence_loss, _ = self.compute_sentence_loss(model, sentence_decoder_out, sample, reduce=reduce)
        # doc_loss, _ = self.compute_doc_loss(model, doc_decoder_out, sample, reduce=reduce)
        ...
        return loss, sentence_loss, 0, sample_size, logging_output
    ...
    #     lprobs = lprobs.view(-1, lprobs.size(-1))
    #     target = sample['sentence_target'].view(-1)
    #     loss = F.nll_loss(lprobs, target, size_average=False, ignore_index=6, reduce=reduce)
    #     return loss, loss
    ...
    def compute_doc_loss(self, model, net_output, sample, reduce=True):
        lprobs = model.doc_decoder.get_normalized_probs(net_output, log_probs=True, sample=sample)
        lprobs = lprobs.view(-1, lprobs.size(-1))
        # target = sample['doc_target'][:, 0].view(-1)
        target = sample['doc_target']
        batch_size = target.size(0)
        max_doc = target.size(1)
        loss = 0
        doc_lengths = sample['net_input']['doc_lengths']
        for i in range(batch_size):
            for j in range(doc_lengths[i]):
                ids = target[i, j:doc_lengths[i]]
                max_l = torch.max(ids)
                if max_l >= lprobs.size(1):
                    print('oh')
                lprobs_for_ids = torch.gather(lprobs[i, :], dim=0, index=ids)
                try:
                    tmp = self.MultiMarginLoss(lprobs_for_ids, torch.LongTensor([0]).cuda())
                    loss += tmp
                except:
                    print('oh')
                # if(tmp>10):
