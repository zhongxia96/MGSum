...
@register_model('hierarchical_transformer_word')
class HierarchicalTransformerModel(FairseqModel):
    """
    hierarchical_transformer_word model from `"Attention Is All You Need" (Vaswani, et al, 2017)
    <https://arxiv.org/abs/1706.03762>`_.

    Args:
        encoder (hierarchical_transformer_wordEncoder): the encoder
        decoder (hierarchical_transformer_wordDecoder): the decoder

    The hierarchical_transformer_word model provides the following named architectures and
    command-line arguments:

    .. argparse::
        :ref: fairseq.models.hierarchical_transformer_word_parser
        :prog:
    """
...
class HierarchicalTransformerEncoder(FairseqEncoder):
    """
    hierarchical_transformer_word encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`hierarchical_transformer_wordEncoderLayer`.
...
@register_model_architecture('hierarchical_transformer_word', 'hierarchical_transformer_word')
...
@register_model_architecture('hierarchical_transformer_word', 'hierarchical_transformer_word_test')
def hierarchical_transformer_word_iwslt_de_en_test(args):
...
@register_model_architecture('hierarchical_transformer_word', 'hierarchical_transformer_word_small')
def hierarchical_transformer_word_iwslt_de_en_test(args):
...
@register_model_architecture('hierarchical_transformer_word', 'hierarchical_transformer_word_medium')
def hierarchical_transformer_word_iwslt_de_en_test(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 2048)
