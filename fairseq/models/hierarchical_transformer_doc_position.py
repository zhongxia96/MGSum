...
@register_model('hierarchical_transformer_doc_position')
class HierarchicalTransformerModel(FairseqModel):
    """
    hierarchical_transformer_doc_position model from `"Attention Is All You Need" (Vaswani, et al, 2017)
    <https://arxiv.org/abs/1706.03762>`_.

    Args:
        encoder (hierarchical_transformer_doc_positionEncoder): the encoder
        decoder (hierarchical_transformer_doc_positionDecoder): the decoder

    The hierarchical_transformer_doc_position model provides the following named architectures and
    command-line arguments:

    .. argparse::
        :ref: fairseq.models.hierarchical_transformer_doc_position_parser
        :prog:
    """
...
class HierarchicalTransformerEncoder(FairseqEncoder):
    """
    hierarchical_transformer_doc_position encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`hierarchical_transformer_doc_positionEncoderLayer`.
    ...
    def forward(self, src_tokens, src_lengths, block_mask, doc_lengths, doc_block_mask):
        ...
        block_pos_emb = collate_embedding([torch.cat([block_pos_emb[i, :doc_sentence_lengths[i, j]]
             for j in range(n_docs) if doc_sentence_lengths[i, j] != 0], 0) for i in
             range(block_pos_emb.size(0))], 0, n_blocks)  # (batch, n_blocks, embed_dim)
        ...
        doc_pos_emb = collate_embedding([torch.cat([doc_pos_emb[i, j].unsqueeze(0).repeat(doc_sentence_lengths[i, j], 1)
             for j in range(doc_pos_emb.size(1)) if doc_sentence_lengths[i, j] != 0], 0) for i in
...
@register_model_architecture('hierarchical_transformer_doc_position', 'hierarchical_transformer_doc_position')
...
@register_model_architecture('hierarchical_transformer_doc_position', 'hierarchical_transformer_doc_position_test')
def hierarchical_transformer_doc_position_iwslt_de_en_test(args):
...
@register_model_architecture('hierarchical_transformer_doc_position', 'hierarchical_transformer_doc_position_small')
def hierarchical_transformer_doc_position_iwslt_de_en_test(args):
...
@register_model_architecture('hierarchical_transformer_doc_position', 'hierarchical_transformer_doc_position_medium')
def hierarchical_transformer_doc_position_iwslt_de_en_test(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 2048)
